...
const ClientsPage = () => {
  ...
  const [projects, setProjects] = useState([]); // sourced from the real `projects` table now
  ...
  // IMPORTANT: this now reads from the real `projects` table (the same one
  // used by the sidebar Projects page, Dashboard, and Kanban board) instead
  // of the old, disconnected `client_projects` table.
  ...
  const deleteClient = async (id) => {
    ...
    if (!confirm("Delete this client? Linked projects will be kept but unlinked from this client.")) return;
  ...
  const getProjectProgress = (clientId) => {
    ...
    return Math.round(cp.reduce((sum, p) => sum + (p.progress || 0), 0) / cp.length);
  };
                    ...
                    return (
                      <TableRow key={c.id}>
                        ...
                        <TableCell>
                          <button
                            type="button"
                            className="flex items-center gap-2 hover:text-primary transition-colors disabled:cursor-default"
                            disabled={clientProjects.length === 0}
                            onClick={() => clientProjects[0] && navigate(`/projects/${clientProjects[0].id}`)}
                          >
                            <FolderOpen className="h-3.5 w-3.5 text-muted-foreground" />
                            <span className="text-sm">{clientProjects.length}</span>
                            {clientProjects.length > 0 && <Progress value={getProjectProgress(c.id)} className="w-16 h-1.5" />}
                          </button>
                        </TableCell>
                        <TableCell>
